#!/usr/bin/env python3
"""
Script de comunicação com a placa "controle" do Exp2_PSI3422 (PSI3422).

Repassa cada tecla digitada no terminal, sem esperar Enter, direto
para a UART da placa (que já entende w/a/s/d/x/q/o — ver
controle/src/main.c) e imprime de volta qualquer linha que a placa
mandar (as linhas de telemetria "dist=... dutyL=... dutyR=...").
...
def main() -> int:
    ...
    print("=== PSI3422 Exp2_PSI3422 -- controle_serial.py ===")
    print(f"Porta: {args.port} @ {args.baud}")
    print("w/a/s/d = mover, x/espaço = frear, q = ponto morto, o = modo automático")

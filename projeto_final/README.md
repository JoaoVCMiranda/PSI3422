...
- ~~**`nrf24_init()` sempre retornava -134 (-ENOTSUP)**~~ RESOLVIDO
  ...
  **sempre falhava**, e o `if (ret < 0) return ret;` logo depois
  ...
  inicializado.
- ~~**Dois pinmaps (`pinmap.yaml` vs `pinmap_carrinho.yaml`)**~~
  RESOLVIDO (sessão de 22/09/2026): a hipótese era que a PCB do
  Carrinho tinha o nRF24 fiado em PTD6/PTB9/PTD7 (commit 6e70945),
  diferente do Controle (fiado à mão em PTC5/PTC6/PTC7, o que
  `pinmap.yaml` atual documenta). `nrf24_read_status()` confirmou:
  status travado em `0xFF` nos DOIS boards — SPI0 de hardware só
  existe em PORTC/PORTA nesta família de chip, então PTD6/PTB9/PTD7
  nunca poderia funcionar por SPI0 em nenhum dos dois. Os fios do
  módulo nRF24 foram movidos fisicamente pra PTC5/PTC6/PTC7 nos dois
  boards — `debug_projeto_final`, `debug_projeto_final_controle` e
  `projeto_final/Carrinho` voltaram a usar `pinmap.h` (não
  `pinmap_carrinho.h`). `pinmap_carrinho.yaml`/`.h`/`.md`/símbolo
  KiCad ficam só como registro histórico do que a PCB tinha antes do
  rework, não são mais a referência ativa. `tools/gen_pinmap.py`
  continua aceitando um YAML alternativo como argumento, caso sirva
  de novo no futuro.
